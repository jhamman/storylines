...
def date_range(start=None, end=None, periods=None, freq='D', tz=None,
    ...
    if (start is not None) and (end is not None) and (periods is None):
        ...
        periods = int((end_num - start_num) / steps) + 1  # Todo divide by freq
        ...
        return pd.DatetimeIndex(xr.conventions.nctime_to_nptime(times))
    elif (start is not None) and (end is None) and (periods is not None):
        units = units_from_freq(freq, start)
        return pd.DatetimeIndex(xr.conventions.nctime_to_nptime(
            num2date(np.arange(periods), units, calendar=calendar)))
